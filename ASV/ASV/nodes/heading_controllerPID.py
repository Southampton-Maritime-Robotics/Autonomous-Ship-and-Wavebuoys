...
#               Boolean published by the circle and zigzag manoeuvres. First-order
#               filter for derivative added.
#
##############################################################################
#Notes
#
...
Pgain      =  2.1991  ##These values have been obtained theoretically - see report
Igain	   =  0.0294
Dgain	   =  1.7743
...
#### Finding T_f for the derivative filter
N = 2.1233
T_f = Dgain / (Pgain * N)
...
def main_control_loop():
	...
	Dterm 		 = 0
        ...
        while not (rospy.is_shutdown() and manoeuvring == 1):
            ...
            ### DERIVATIVE ###
            ##Implementation of a first-order filter as well##
            second_term = Dgain *(error-error_previous)/ (T_f + dt)  
                
            ############ALGORITHM TO CALCULATE RUDDER RESPONSE##############
            Pterm = Pgain * error 
            Iterm = Igain * int_error
            Dterm = Dterm * T_f / (T_f + dt) - second_term
            ...
            error_previous= error	       	     ##This updates the value of the error_zero to that of the previous time
