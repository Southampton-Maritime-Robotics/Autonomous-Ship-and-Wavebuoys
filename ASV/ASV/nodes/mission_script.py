...
################################################################################
        ...
        smach.StateMachine.add('INITIALISE', Initialise(lib,15), #(lib, timeout)	This state ensure all systems are running correctly proir to starting the mission proper
            transitions={'succeeded':'WAYPOINT1', 'aborted':'STOP','preempted':'STOP'}) #In the original, 'succeeded':'WAYPOINT1' 
            
        smach.StateMachine.add('WAYPOINT1', GoToXY(lib, 5, 0, 1, 30),  #(lib, X, Y, Radial tolerance,timeout)
            transitions={'succeeded':'WAYPOINT2', 'aborted':'STOP','preempted':'STOP'})            
    
        smach.StateMachine.add('WAYPOINT2', GoToXY(lib, 5, 10, 1, 20),  #(lib, X, Y, Radial tolerance,timeout)
            transitions={'succeeded':'CIRCLE', 'aborted':'STOP','preempted':'STOP'})     #In the original, 'succeeded': 'STOP'
                               
        smach.StateMachine.add('CIRCLE', circle(lib, 1100, -35, 100),  
